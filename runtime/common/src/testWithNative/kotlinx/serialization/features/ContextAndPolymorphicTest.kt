...
class ContextAndPolymorphicTest {
    ...
    @Test
    fun testContextSerializerHasSpecialDescriptor() {
        val desc = EnhancedData.serializer().descriptor.getElementDescriptor(1)
        assertEquals(UnionKind.CONTEXT, desc.kind)
        assertEquals("Payload", desc.name)
    }
}
